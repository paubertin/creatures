import { clamp, euclideanModulo, lerp } from './utils';
...
export class Color {
  ...
  constructor(r: number | [number, number, number] | IColor, g?: number, b?: number) {
    if (Array.isArray(r)) {
      return this.setRGB(...r);
    }
  ...
  setStyle(style: string) {
    function handleAlpha(string?: string) {
      ...
      if (parseFloat(string) < 1) {
        console.warn(`Alpha component of ${ style } will be ignored.`);
      }
    }
    ...
    if (style && style.length > 0) {
      return this.setColorName(style as keyof typeof COLORS);
    }

    return this;
  }

  setColorName(style: keyof typeof COLORS) {
    // color keywords
    const hex = COLORS[style];
